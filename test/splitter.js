...
contract('Splitter', function (accounts) {
  ...
  it("should throw if person's balance is zero ", async function () {
    await expectThrow(contract.withdraw({
      from: owner
    }));
  })

  it("should withdraw person's balance", async function () {
    await contract.splitValue({
      from: owner,
      value: oddValueSend
    });
  ...
  it("should throw if the sender is not the owner", async function () {
    await expectThrow(contract.splitValue({
      from: accounts[4],
      value: valueSend
    }));
  });
